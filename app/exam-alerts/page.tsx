...
function RegionSection({ title, description, alerts }: { title: string; description: string; alerts: ExamAlert[] }) {
  return <section aria-labelledby={title.replaceAll(" ", "-").toLowerCase()} className="mt-10"><div className="mb-4 flex items-start gap-3"><div className="rounded-xl bg-primary/10 p-3 text-primary"><MapPin className="h-5 w-5" /></div><div><h2 id={title.replaceAll(" ", "-").toLowerCase()} className="text-2xl font-bold">{title}</h2><p className="mt-1 text-sm text-muted-foreground">{description}</p></div></div>{alerts.length ? <div className="space-y-4">{alerts.map((alert) => <AlertCard key={alert.id} alert={alert} />)}</div> : <Card><CardContent className="py-10 text-center text-sm text-muted-foreground">No published {title} notices found in the live database.</CardContent></Card>}</section>
}
