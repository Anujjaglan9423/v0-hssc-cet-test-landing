...
export const metadata: Metadata = {
  ...
  description: "Live government exam notifications for Haryana (HSSC/HPSC), Uttarakhand (UKSSSC/UKPSC), Railway RRB and SSC. Verified links to official sources, updated daily.",
...
export default async function ExamAlertsPage() {
  ...
  return <><FooterLinkNavbar /><main className="min-h-screen bg-background px-4 pb-16 pt-24 sm:px-6 lg:px-8"><div className="mx-auto max-w-5xl"><header className="mb-8"><div className="flex items-start gap-3"><div className="rounded-xl bg-primary/10 p-3 text-primary"><BellRing className="h-6 w-6" /></div><div><p className="text-sm font-semibold text-primary">Live official updates</p><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Government Exam Alerts</h1><p className="mt-2 max-w-3xl text-muted-foreground">Find verified government exam notifications by category. Each notice links to its official government source.</p><p className="mt-3 text-xs text-muted-foreground">Verified updates from official government sources · Last updated: {fetchedAt.toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short", timeZone: "Asia/Kolkata" })} IST</p></div></div></header><ExamAlertsBrowser alerts={alerts} /><section aria-labelledby="exam-alerts-faq" className="mt-12 rounded-2xl border bg-card/50 p-6"><h2 id="exam-alerts-faq" className="text-2xl font-bold">Exam alerts FAQ</h2><div className="mt-5 flex flex-col gap-5 text-sm"><div><h3 className="font-semibold">How often are exam alerts updated on CET TEST?</h3><p className="mt-1 text-muted-foreground">Exam alerts are checked and updated daily from official government sources including HSSC, UKSSSC, RRB and SSC.</p></div><div><h3 className="font-semibold">Which states and exams does CET TEST cover?</h3><p className="mt-1 text-muted-foreground">CET TEST covers Haryana, Uttarakhand, Railway and SSC exam notifications.</p></div><div><h3 className="font-semibold">Are these exam alerts official?</h3><p className="mt-1 text-muted-foreground">Each alert links directly to the official government source page so candidates can verify and download the original notice.</p></div></div></section></div></main><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><Footer /></>
}
