    ...
    <>
      ...
      <div className="mx-auto max-w-5xl">
        ...
        {alerts.length === 0 ? (
          ...
          <div className="space-y-4">{alerts.map((alert) => <Card key={alert.id}><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start sm:justify-between"><div><div className="mb-2 flex flex-wrap items-center gap-2"><Badge variant="secondary">{alert.category || "Exam update"}</Badge><span className="text-xs text-muted-foreground">{new Date(alert.createdAt).toLocaleDateString("en-IN")}</span></div><h2 className="text-lg font-semibold">{alert.title}</h2>{alert.description && <p className="mt-2 text-sm leading-6 text-muted-foreground">{alert.description}</p>}</div><Link href={alert.sourceUrl || `/blog/${alert.slug}`} target={alert.sourceUrl ? "_blank" : undefined} rel={alert.sourceUrl ? "noreferrer" : undefined} className="inline-flex shrink-0 items-center gap-2 text-sm font-medium text-primary hover:underline">View official notice <ExternalLink className="h-4 w-4" /></Link></CardContent></Card>)}</div>
        )}
      </div>
      </main>
      <Footer />
