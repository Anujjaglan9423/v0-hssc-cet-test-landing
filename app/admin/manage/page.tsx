...
import { FolderTree, BookOpen, FileText, Tag, Plus, Edit, Trash2, Loader2, Save, Search } from "lucide-react"
...
function ManageContent() {
  ...
  async function loadAllData() {
    setIsLoading(true)
    try {
      const [categoriesData, examsData, subjectsData, topicsData] = await Promise.all([
        getExamCategories(),
        getAllExams(),
        getAllSubjects(),
        getAllTopics(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Manage Categories & Structure</h1>
          <p className="text-muted-foreground mt-1">
            Create, edit, and delete exam categories, exams, subjects, and topics
          </p>
        </div>
              ...
              <Card key={category.id} className="hover:shadow-lg transition-shadow">
                ...
                <CardContent>
                  ...
                  <div className="mt-3 flex items-center text-xs text-muted-foreground">
                    ...
                    {exams.filter((e) => e.category_id === category.id).length} exams
                  </div>
                </CardContent>
              </Card>
            ))}
                ...
                <Card key={exam.id} className="hover:shadow-lg transition-shadow">
                  ...
                  <CardContent>
                    ...
                    <div className="mt-3 flex items-center text-xs text-muted-foreground">
                      ...
                      {subjects.filter((s) => s.exam_id === exam.id).length} subjects
                    </div>
                  </CardContent>
                </Card>
              )
                ...
                <Card key={topic.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleEdit(topic)}>
                          <Edit className="w-4 h-4" />
                        </Button>
                        <Button
                          variant="ghost"
      ...
      <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Fill in the details to create a new {getTabLabel().toLowerCase()}</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <Button variant="outline" onClick={() => setShowAddModal(false)} className="bg-transparent">
              Cancel
            </Button>
            <Button onClick={handleSubmitAdd} disabled={isSubmitting || !formData.name} className="gap-2">
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Update the {getTabLabel().toLowerCase()} details</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button onClick={handleSubmitEdit} disabled={isSubmitting || !formData.name} className="gap-2">
              {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
              Save Changes
            </Button>
          </div>
