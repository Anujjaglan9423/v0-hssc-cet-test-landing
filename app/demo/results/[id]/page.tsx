...
export default function DemoResultsPage() {
  ...
  const getGrade = (percentage: number) => {
    ...
    if (percentage >= 60) return { grade: "B", color: "text-blue-500", bg: "bg-blue-500/10" }
    if (percentage >= 50) return { grade: "C", color: "text-amber-500", bg: "bg-amber-500/10" }
      ...
      <section className="pt-24 pb-16 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            <div className={`w-24 h-24 rounded-full ${gradeInfo.bg} flex items-center justify-center mx-auto mb-6`}>
          ...
          <Card className="p-6 md:p-8 mb-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
              <div className="text-center">
                <div className={`w-16 h-16 rounded-full ${gradeInfo.bg} flex items-center justify-center mx-auto mb-3`}>
              ...
              <div className="text-center">
                ...
                <p className="text-lg font-bold text-foreground">{formatTime(results.timeTaken)}</p>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Progress value={(results.correct / results.totalQuestions) * 100} className="h-2 bg-accent/20" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                ...
                <Progress value={(results.wrong / results.totalQuestions) * 100} className="h-2 bg-destructive/20" />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                ...
                <Progress value={(results.unanswered / results.totalQuestions) * 100} className="h-2 bg-muted" />
              </div>
            </div>
          </Card>
                  ...
                  <div key={q.id} className="border border-border rounded-lg overflow-hidden">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <Badge
                          variant={
                            result.isCorrect ? "default" : result.selected === null ? "secondary" : "destructive"
                          }
                        >
                          {result.isCorrect ? "Correct" : result.selected === null ? "Skipped" : "Wrong"}
                        </Badge>
                            ...
                            <div
                              ...
                              className={`p-3 rounded-lg border ${
                                opt.key === result.correct
                                  ...
                                  : opt.key === result.selected && !result.isCorrect
                                    ? "border-destructive bg-destructive/10"
                                    : "border-border"
                              }`}
                            >
                              <span className="font-medium uppercase mr-2">{opt.key}.</span>
                              {opt.value}
                              {opt.key === result.correct && (
          ...
          <Card className="p-8 bg-gradient-to-br from-primary/10 via-accent/10 to-primary/5 border-primary/20">
            <div className="text-center space-y-6">
              ...
              <div>
                <h2 className="text-2xl font-bold text-foreground mb-2">Enjoyed This Demo?</h2>
                <p className="text-muted-foreground max-w-lg mx-auto">
                  Sign up now to access 10,000+ practice questions, full-length mock tests, detailed analytics, and
                  personalized study recommendations to crack your HSSC CET exam!
                </p>
              </div>
